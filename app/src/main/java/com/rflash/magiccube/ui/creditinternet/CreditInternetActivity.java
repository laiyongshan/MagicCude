...
public class CreditInternetActivity extends MVPBaseActivity<CreditInternetContract.View, CreditInternetPresenter> implements CreditInternetContract.View, SwipeRefreshLayout.OnRefreshListener, LoadRecyclerView.OnLoadMoreListener, CreditInternetAdapter.PersonalMessageListener, CreditInternetAdapter.QueryReportListener {
    ...
    private CreditInternetAdapter creditInternetAdapter;
    ...
    private void initView() {
        toolbar.setTitle("云信贷互联反欺诈");
        toolbar.setNavigationOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                CreditInternetActivity.this.finish();
            }
        });
        ...
        //创建CreditInternetAdapter
        creditInternetAdapter = new CreditInternetAdapter(this, list, R.layout.layout_credit_query_share);
        ...
        //设置creditInternetAdapter
        recyclerView.setAdapter(creditInternetAdapter);
        //设置billAdapter 的点击监听
        creditInternetAdapter.setListener(this);
        creditInternetAdapter.setReportListener(this);
//        mPresenter.queryCreditInternet(Config.CLOUD_CREDIT_INTERNET, "1");
    }
    ...
    public void onLoad() {
        if (totalPage >= pageNum + 1) {
            mPresenter.queryCreditInternet(Config.CLOUD_CREDIT_INTERNET, ++pageNum + "");
        } else {
    ...
    public void onViewClicked(View view) {
        switch (view.getId()) {
            case R.id.btn_recharge://充值次数
                ...
                bundleCharge.putString("total","云信贷互联反欺诈");
                bundleCharge.putString("creditType", Config.CLOUD_CREDIT_INTERNET);
            ...
            case R.id.btn_query://查询
                ...
                bundle.putString("total", "云信贷互联反欺诈");
                bundle.putString("creditType", Config.CLOUD_CREDIT_INTERNET);
     ...
     * 云信贷互联反欺诈列表
     *
     * @param creditInternet
     */
    @Override
    public void getCreditInternet(CreditShareItem creditInternet) {
        totalPage = creditInternet.getTotalPage();
        List<CreditShareItem.ResultBean> result = creditInternet.getResult();
        if(StringUtil.isEmpty(creditInternet.getTimes())){
            tv_over_times.setText("剩余次数：0");
            count = "0";
        }else{
            count = creditInternet.getTimes();
        ...
        creditInternetAdapter.notifyDataSetChanged();
        refresh = false;
    }
