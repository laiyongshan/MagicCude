...
public class SuspectCashActivity extends MVPBaseActivity<SuspectCashContract.View, SuspectCashPresenter> implements SuspectCashContract.View, SwipeRefreshLayout.OnRefreshListener, LoadRecyclerView.OnLoadMoreListener, SuspectCashAdapter.PersonalMessageListener, SuspectCashAdapter.QueryReportListener {
    ...
    private SuspectCashAdapter suspectCashAdapter;
    ...
    private void initView() {
        toolbar.setTitle("用户疑似套现");
        toolbar.setNavigationOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                SuspectCashActivity.this.finish();
            }
        });
        ...
        //创建suspectCashAdapter
        suspectCashAdapter = new SuspectCashAdapter(this, list, R.layout.layout_credit_query_share);
        ...
        //设置suspectCashAdapter
        recyclerView.setAdapter(suspectCashAdapter);
        //设置suspectCashAdapter 的点击监听
        suspectCashAdapter.setListener(this);
        suspectCashAdapter.setReportListener(this);
//        mPresenter.querySuspectCash(Config.SUSPECT_CASH, "1");
    }
    ...
    public void onLoad() {
        if (totalPage >= pageNum + 1) {
            mPresenter.querySuspectCash(Config.SUSPECT_CASH, ++pageNum + "");
        } else {
    ...
    public void onViewClicked(View view) {
        switch (view.getId()) {
            case R.id.btn_recharge://充值次数
                ...
                bundleCharge.putString("total","用户疑似套现");
                bundleCharge.putString("creditType", Config.SUSPECT_CASH);
            ...
            case R.id.btn_query://查询
                ...
                bundle.putString("total", "用户疑似套现");
                bundle.putString("creditType", Config.SUSPECT_CASH);
     ...
     */
    @Override
    public void getSuspectCash(CreditShareItem suspectCash) {
        totalPage = suspectCash.getTotalPage();
        List<CreditShareItem.ResultBean> result = suspectCash.getResult();
        if(StringUtil.isEmpty(suspectCash.getTimes())){
            tv_over_times.setText("剩余次数：0");
            count = "0";
        }else{
            count = suspectCash.getTimes();
        ...
        suspectCashAdapter.notifyDataSetChanged();
        refresh = false;
    }
