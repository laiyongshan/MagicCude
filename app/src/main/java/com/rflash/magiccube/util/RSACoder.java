...
public class RSACoder {
    ...
    public static String signMSWithKeyStr(String data, String privateKeyStr) throws Exception { 
    	PrivateKey privateKey = getPrivateKeyStr(privateKeyStr);
        // ����PKCS8EncodedKeySpec����  
        PKCS8EncodedKeySpec pkcs8KeySpec = new PKCS8EncodedKeySpec(privateKey.getEncoded());    
        // KEY_ALGORITHM ָ���ļ����㷨  
        KeyFactory keyFactory = KeyFactory.getInstance(Algorithm);   
    ...
    private static PrivateKey getPrivateKeyStr(String keyStr) throws Exception {
    	byte[] keyBytes = buildPKCS8Key(keyStr);
        PKCS8EncodedKeySpec keySpec = new PKCS8EncodedKeySpec(keyBytes);
        KeyFactory keyFactory = KeyFactory.getInstance(Algorithm);
        PrivateKey privateKey = keyFactory.generatePrivate(keySpec);
        return privateKey;
    ...
    public static boolean verifyMSWithStr(String data, String signData, String publicKeyStr)  
            throws Exception {
    	PublicKey publicKey = getPubliceKeyStr(publicKeyStr);
        // ����X509EncodedKeySpec����  
        X509EncodedKeySpec keySpec = new X509EncodedKeySpec(publicKey.getEncoded());    
        // KEY_ALGORITHM ָ���ļ����㷨  
        KeyFactory keyFactory = KeyFactory.getInstance(Algorithm);    
    ...
    public static PublicKey getPubliceKeyStr(String keyStr) throws Exception {
    	String key = keyStr;
        key = key.replaceAll("\\-{5}[\\w\\s]+\\-{5}[\\r\\n|\\n]", "");
        KeyFactory keyFactory = KeyFactory.getInstance(Algorithm);
        byte[] encodedKey = Base64.decode(key);
